  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        const response = await axios.get(`${Environment.BaseAPIURL}/api/User/Get_MenuBindByRoleWise?UserId=${encryptData(userId)}&ModuleId=${moduleId}`, {
          headers: {
            ...
            <aside id="sidebar" className={sidebarOpen ? 'sidebar-open' : 'SideBarSection'}>
              ...
              <ul>
                {permissions.map((module, index) => {
                  const isModuleLink = module.menuName === "Module";

                  return (
                    <li key={index}>
                      <Link to={isModuleLink ? `${module.link}` : `${module.link}?moduleId=${moduleId}&menuId=${encryptData(module.menuId)}`} className="RegisterEmployeeBtn">
                ...
                {/* {permissions.map((module, index) => (
                  <li key={index}>
                    <Link to={`${module.link}?moduleId=${moduleId}&menuId=${encryptData(module.menuId)}`} className="RegisterEmployeeBtn">
                      <span><i className={module.icon}></i> {module.menuName}</span>
                    </Link>
                  </li>
                ))} */}
