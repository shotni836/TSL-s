...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      let deviceId = secureLocalStorage.getItem('deviceId');
      if (!deviceId) {
        deviceId = uuidv4(); // Generate a unique deviceId
      }

      const response = await axios.post(`${Environment.BaseAPIURL}/api/User/AuthLogin?username=${encryptData(username)}&password=${encryptData(password)}&deviceId=${encryptData(deviceId)}`);
  ...
  function completeRegister(e) {
    e.preventDefault()
    navigate(`/completeregister?UserId=${encryptData(myUserId)}`)
  }
