...
function FieldTesting() {
    ...
    const fetchEditDetails = async () => {
        // try {
        setLoading(true);
        const response = TestId == '987' || TestId == '1433' ? await axios.get(`${Environment.BaseAPIURL}/api/User/GetPeeltestDataById?ProcessSheetID=${procSheetId}&ProcessSheetTypeID=${ProcessSheetTypeID}&TestRunId=${TestRunId}&TestId=${TestId}`)
            : TestId == '1525' || TestId == '1526' ? await axios.get(`${Environment.BaseAPIURL}/api/User/GetTrialtestById?ProcessSheetID=${procSheetId}&ProcessSheetTypeID=${ProcessSheetTypeID}&TestRunId=${TestRunId}&test_id=${TestId}`)
