...
function BeforeProcessLabTesting() {
    ...
    const fetchEditDetails = async () => {
        try {
            setLoading(true);
            const response = await axios.get(Environment.BaseAPIURL + `/api/User/GetBeforeDetailsById?ProcessSheetID=${procSheetId}&ProcessSheetTypeID=${ProcessSheetTypeID}&TestRunId=${TestRunId1}`, {
                headers: {
    ...
    const handleBeforeProcessSelect = async (event) => {
        ...
        try {
            const response = await axios.get(Environment.BaseAPIURL + `/api/User/GetBeforeProcessLabTesting?processsheetno=${encryptData(processSeqNo.current)}&testid=${encryptData(event.target.value)}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
