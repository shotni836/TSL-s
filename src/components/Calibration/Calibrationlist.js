...
function Calibrationlist() {
  ...
  const fetchData = async () => {
    try {
      const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetPermissionDetailsByPageId`, {
        params: { UserId: userId, PageId: 30 }
      });
  ...
  const handleViewClick = (row) => {
    navigate(`/calibrationview?id=${row.calib_id}&pm_Approve_level=view&menuId=${30}`, '_blank');
  };

  const handleEdit = (row) => {
    navigate(`/Calibration?action=edit&id=${row.calib_id}&menuId=${30}`, '_blank');
  };

  const handleFirstLevel = (row) => {
    navigate(`/calibrationview?id=${row.calib_id}&pm_Approve_level=first&menuId=${30}`, '_blank');
  };

  const handleSecondLevel = (row) => {
    navigate(`/calibrationview?id=${row.calib_id}&pm_Approve_level=second&menuId=${30}`, '_blank');
  };
                  ...
                  <div className='RawmateriallistTables'>
                    ...
                    <div className='tableheaderflex'>
                      ...
                      <div className='tableheaderAddbutton'>
                        <Link to={`/Calibration?menuId=${30}`} style={{ float: 'right' }}>
