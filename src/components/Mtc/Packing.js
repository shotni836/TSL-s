...
function Packing() {
    ...
    const getDDList = async () => {
        try {
            const response1 = await axios.post(Environment.BaseAPIURL + `/api/User/getEPOXYProcessSheetDetails?processsheetno=${encryptData(formData.psSeqNo)}&year=${encryptData(formData.psYear)}`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            })
            ...
            const response = await axios.get(Environment.BaseAPIURL + `/api/User/GetSoNoforCPRL?pm_procsheet_id=${encryptData(formData?.psSeqNo)}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            })
    ...
    const handleSearchClick = async (e) => {
        ...
        try {
            ...
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetMainDataForCPRL?pm_procsheet_id=${encryptData(procesSheetNo)}&pm_salesord_no=${encryptData(soNoValue)}&pm_item_no=${encryptData(soLineNoValue)}&pm_pono=${encryptData(encodeURIComponent(poNo))}&Coatingdate=${encryptData(new Date(selectedDates).toLocaleDateString("fr-CA").replace(/\//g, "-"))}&pm_po_item_no=${encryptData(poNoValue)}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
                            ...
                            <div className="row">
                                <div className="col-md-12 col-sm-12 col-xs-12">
                                    <ul>
                                        ...
                                        <b style={{ color: '#fff' }}>/ &nbsp;</b>
                                        <li> <Link to={`/packinglist?moduleId=${moduleId}&menuId=${menuId}`}> Packing List </Link> <b style={{ color: '#fff' }}></b></li>
                                        <li><h1>/&nbsp; Packing </h1></li>
                                    </ul>
                                </div>
                            </div>
