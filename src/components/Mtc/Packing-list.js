...
function Packinglist() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetPackinglistdetails?userid=${encryptData(userId)}&rtype=${encryptData(1)}`, {
                headers: {
    ...
    const handleViewClick = (row) => {
        navigate(`/packingreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_cprl_run_id)}&pm_Approve_level=view&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleEditOrder = (row) => {
        navigate(`/packingreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_cprl_run_id)}&pm_Approve_level=editOrder&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    }

    const handleEdit = (row) => {
        navigate(`/packing?moduleId=${moduleId}&menuId=${menuId}&action=edit&run_id=${encryptData(row.pm_cprl_run_id)}&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleHod = (row) => {
        navigate(`/packingreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_cprl_run_id)}&pm_Approve_level=hod&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleFirstLevel = (row) => {
        navigate(`/packingreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_cprl_run_id)}&pm_Approve_level=first&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleSecondLevel = (row) => {
        navigate(`/packingreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_cprl_run_id)}&pm_Approve_level=second&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };
                                    ...
                                    <div className='RawmateriallistTables'>
                                        ...
                                        <div className='tableheaderflex'>
                                            ...
                                            <div className='tableheaderAddbutton'>
                                                <Link to={`/packing?moduleId=${moduleId}&menuId=${menuId}`} style={{ float: 'right' }}>
