...
function Mtclist() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetPackinglistdetails?userid=${userId}&rtype=2`);
    ...
    const handleViewClick = (row) => {
        navigate(`/mtcreport?id=${row.pm_mtc_run_id}&pm_Approve_level=view&pm_processSheet_id=${row.procsheet_id}&menuId=${menuId}`, '_blank');
    };

    const handleEdit = (row) => {
        navigate(`/mtcreport?action=edit&run_id=${row.pm_mtc_run_id}&pm_processSheet_id=${row.procsheet_id}&menuId=${menuId}`, '_blank');
    };

    const handleFirstLevel = (row) => {
        navigate(`/mtcreport?id=${row.pm_mtc_run_id}&pm_Approve_level=first&pm_processSheet_id=${row.procsheet_id}&menuId=${menuId}`, '_blank');
    };

    const handleSecondLevel = (row) => {
        navigate(`/mtcreport?id=${row.pm_mtc_run_id}&pm_Approve_level=second&pm_processSheet_id=${row.procsheet_id}&menuId=${menuId}`, '_blank');
    };
                                    ...
                                    <div className='RawmateriallistTables'>
                                        ...
                                        <div className='tableheaderflex'>
                                            ...
                                            <div className='tableheaderAddbutton'>
                                                <Link to={`/mtcreport?menuId=${menuId}`} style={{ float: 'right' }}>
