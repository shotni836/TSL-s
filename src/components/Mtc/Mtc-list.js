...
function Mtclist() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetPackinglistdetails?userid=${encryptData(userId)}&rtype=${encryptData(2)}`, {
                headers: {
    ...
    const handleViewClick = (row) => {
        navigate(`/mtcreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_mtc_run_id)}&pm_Approve_level=view&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleEdit = (row) => {
        navigate(`/mtcreport?moduleId=${moduleId}&menuId=${menuId}&action=edit&run_id=${encryptData(row.pm_mtc_run_id)}&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleFirstLevel = (row) => {
        navigate(`/mtcreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_mtc_run_id)}&pm_Approve_level=first&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };

    const handleSecondLevel = (row) => {
        navigate(`/mtcreport?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(row.pm_mtc_run_id)}&pm_Approve_level=second&pm_processSheet_id=${encryptData(row.procsheet_id)}`, '_blank');
    };
                                    ...
                                    <div className='RawmateriallistTables'>
                                        ...
                                        <div className='tableheaderflex'>
                                            ...
                                            <div className='tableheaderAddbutton'>
                                                <Link to={`/mtcreport?moduleId=${moduleId}&menuId=${menuId}`} style={{ float: 'right' }}>
                                        ...
                                        <ul className='Rawmaterialinspectionlistlegend'>
                                            ...
                                            <li><i className="fas fa-check" style={{ color: "#4caf50" }}></i> Second Level Approval</li>
                                        </ul>
