...
const AssignLabFieldTest = () => {
  ...
  const test_run_id1 = decryptData(test_run_id);
  ...
  const fetchEditDetails = async () => {
    try {
      const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetAssignLabFieldDataById?test_run_id=${test_run_id}&type=${encryptData(LabField === "Field" ? "F" : "L")}`, {
        headers: {
  ...
  const getHeaderData = async () => {
    try {
      const processsheetId = action === 'edit' ? processSheetId : encryptData(formData?.processsheetid);
      const yearValue = action === 'edit' ? year : encryptData(formData?.year);
  ...
  const labPipeCoatData = async (id, shift) => {
    try {
      const response = await axios.get(Environment.BaseAPIURL + `/api/User/Assignfieldlabtestonpipe?processsheet_id=${encryptData(formData?.processsheetid || id)}&type=${encryptData(LabField === "Lab" ? "L" : "F")}&shiftid=${encryptData(shift.pm_shift_id)}&testdate=${encryptData(new Date().toLocaleDateString('fr-CA').replace(/\//g, "-").split(" ")[0])}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        }
      });
  ...
  const handleSubmit = (e, value) => {
    ...
    const dataToSend = {
      ...
      testtype: testType,
      test_run_id: action === 'edit' ? test_run_id1 : 0,
    ...
    axios.post(Environment.BaseAPIURL + "/api/User/UpdateAssignfieldlabtestonpipeEntry", dataToSend, {
      ...
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          navigate(`/assignfieldlabtestlist?moduleId=${moduleId}&menuId=${menuId}&test=${LabField}`);
        }
      })
      .catch(error => {
              ...
              <div className="row">
                <div className="col-md-12 col-sm-12 col-xs-12">
                  <ul>
                    ...
                    <b style={{ color: '#fff' }}>/ &nbsp;</b>
                    <li> <Link to={`/inspectiontesting?moduleId=${moduleId}&menuId=${menuId}`}> Raw Material Test & Inspection </Link> <b style={{ color: '#fff' }}></b></li>
                    <b style={{ color: '#fff' }}>/ &nbsp;</b>
                    <li> <Link to={`/assignfieldlabtestlist?moduleId=${moduleId}&menuId=${menuId}&test=${LabField}`}> Assign {LabField} Test On Pipes List </Link> <b style={{ color: '#fff' }}></b></li>
