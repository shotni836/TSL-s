...
function Completeregister() {
  ...
  const submitPersonalDetails = async () => {
    try {
      let personalDetailsPayload = {
        userid: personalDetails.emp_user_id,
  ...
  const submitEducation = async () => {
    try {
      const educationResponse = await axios.post(`${Environment.BaseAPIURL}/api/User/RegEmployeeEducation`, educationData);
  ...
  const submitExperience = async () => {
    try {
      const experienceResponse = await axios.post(`${Environment.BaseAPIURL}/api/User/RegEmployeeExperience`, experienceData);
