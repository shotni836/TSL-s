...
function Listcoatingapplicationline() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetProdApplicationdata?UserId=${userId}`);
  ...
  const handleViewClick = (row) => {
    navigate(`/viewcoatingapplicationline?id=${row.prodAppId}&pm_Approve_level=view&pm_processSheet_id=${row.ProcsheetId}&menuId=${menuId}`, '_blank');
  };

  const handleEdit = (row) => {
    navigate(`/addcoatingapplicationline?menuId=${menuId}&action=edit&id=${row.prodAppId}`, '_blank');
  };

  const handleHodLevel = (row) => {
    navigate(`/viewcoatingapplicationline?id=${row.prodAppId}&pm_Approve_level=hod&pm_processSheet_id=${row.ProcsheetId}&menuId=${menuId}`, '_blank');
  };

  const handleFirstLevel = (row) => {
    navigate(`/viewcoatingapplicationline?id=${row.prodAppId}&pm_Approve_level=first&pm_processSheet_id=${row.ProcsheetId}&menuId=${menuId}`, '_blank');
  };

  const handleSecondLevel = (row) => {
    navigate(`/viewcoatingapplicationline?id=${row.prodAppId}&pm_Approve_level=second&pm_processSheet_id=${row.ProcsheetId}&menuId=${menuId}`, '_blank');
  };
                  ...
                  <div className='RawmateriallistTables'>
                    ...
                    <div className='tableheaderflex'>
                      ...
                      <div className='tableheaderAddbutton'>
                        <Link to={`/addcoatingapplicationline?menuId=${menuId}`} style={{ float: 'right' }}>
