...
<<<<<<< HEAD
  ...
  const handleEdit = (row) => {
    navigate(`/addblastingline?menuId=${menuId}&id=${encryptData(row.ProdBlastingId)}&action=edit`, '_blank');
  };

  const handleHodLevel = (row) => {
    navigate(`/viewblastingline?menuId=${menuId}&id=${encryptData(row.ProdBlastingId)}&pm_Approve_level=hod&pm_processSheet_id=${encryptData(row.ProcsheetId)}`, '_blank');
  };

  const handleFirstLevel = (row) => {
    navigate(`/viewblastingline?menuId=${menuId}&id=${encryptData(row.ProdBlastingId)}&pm_Approve_level=first&pm_processSheet_id=${encryptData(row.ProcsheetId)}`, '_blank');
  };
...
>>>>>>> 0a85340d990666d57c1dc8f53a7afcf047357ac9
                    ...
                    <ul className='Rawmaterialinspectionlistlegend'>
                      ...
                      <li><i className="fas fa-check" style={{ color: "#4caf50" }} ></i>HOD</li>
                      <li><i className="fas fa-check-double" style={{ color: "#4caf50" }} ></i>QA</li>
                      <li><i className="fas fa-check-circle" style={{ color: "#4caf50" }} ></i>TPIs</li>
                    </ul>
