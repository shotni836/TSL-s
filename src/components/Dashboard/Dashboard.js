...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        ...
        const response = await axios.get(`${Environment.BaseAPIURL}/api/User/Get_MenuBindByRoleWiseNew?UserId=${userId}&ModuleId=${moduleId}`);
  ...
  const renderMenu = (menu, level = 1) => {
    ...
    return (
      <li key={menu.menuId}>
        {menu.link ? (
          <Link to={`${menu.link}?menuId=${menu.menuId}`} className="RegisterEmployeeBtn">
