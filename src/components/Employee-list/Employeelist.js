...
function Employeelist() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${Environment.BaseAPIURL}/api/User/getallemp?EMProle=${toggleValue}`);
  ...
  const handleRoleUpdate = async userId => {
    ...
    if (window.confirm("Are you sure you want to update the role?")) {
      try {
        await axios.post(`${Environment.BaseAPIURL}/api/User/roleassingemplyee?empiduserid=${userId}&empyeeroleid=${roleId}`);
        fetchData();
      } catch (error) {
  ...
  const handleSwitchToggle = async (UserId, newStatus) => {
    ...
    try {
      await axios.post(`${Environment.BaseAPIURL}/api/User/modifyuserstatus?P_statusVal=${statusVal}&userid=${UserId}`);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(`${Environment.BaseAPIURL}/api/User/modifyuserstatus?P_statusVal=NC&userid=${selectedUserId}`, {
                              ...
                              <td style={{ color: row.EmpStatus === 0 ? "grey" : row.textColor }}>
                                {permissions.indexPerm === "1" && (
                                  <Link
                                    ...
                                    to={row.update === "1" ? `/updateProfile?UserId=${row.UserId}` : `/profile?view&UserId=${row.UserId}`}
                                    style={{ color: row.Gender !== "" ? "green" : "grey" }} >
                                    <i className="fa-solid fa-eye" style={{ color: row.Gender !== null && row.EmpStatus !== 0 ? "green" : "grey" }} />
                                  </Link>
                                )}
                                {permissions.editPerm === "1" && (<Link to={`/profile?edit&UserId=${row.UserId}`}> <i className="fa-solid fa-edit" /></Link>)}
