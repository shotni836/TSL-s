...
function Externaloriginlist() {
  ...
  const getProcessUrl = (data, action) => {
    return `/externaloriginview?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(data.data.pm_test_method_id)}&pm_Approve_level=${action}`;
  };
  ...
  const handleViewClick = (data) => {
    ...
    navigate(`/externaloriginview?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(data.data.pm_test_method_id)}`, '_blank')
  };

  const handleListClick = (data) => {
    navigate(`/externaloriginedit?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(data.data.pm_test_method_id)}`, '_blank');
  };
