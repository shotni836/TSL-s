...
function ProcessSheetlist() {
  ...
  const handleViewClick = (rowData) => {
    navigate(`/processsheetview?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.project_id)}&Proc_type=${encryptData(rowData.Proc_Type)}&procSheetId=${encryptData(rowData?.pm_procsheet_id)}&ViewType=View`);
  };

  const handleEditClick = (rowData) => {
    navigate(`/coating?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.project_id)}&Proc_type=${encryptData(rowData.Proc_Type)}`);
  };

  const handleCopyClick = (rowData) => {
    navigate(`/coating?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.project_id)}&Proc_type=${encryptData(rowData.Proc_Type)}&isProcessSheetCopy=true`);
  };
  ...
  const handleDeleteConfirmation = (rowData, index, closeToast) => {
    ...
    axios.get(Environment.BaseAPIURL + `/api/User/Delete?project_id=${encryptData(rowData.project_id)}&proc_type=${encryptData(rowData.Proc_Type)}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const onhandleAddStationClick = (rowData) => {
    navigate(`/addstation?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.pm_procsheet_id)}&proc_type=${encryptData(rowData.Proc_Type)}&coatingType=${encryptData(rowData.Coatingtype)}&processSheet=${encryptData(rowData.ProcSheetNo)}&client=${encryptData(rowData.Client)}&LOIPOFOA=${encryptData(rowData["LOI/PO/FOA No"])}&coatingDate=${encryptData(rowData.ProcSheetDate)}`);
  };

  const [ShowTestDate, setShowTestDate] = useState("");
  const onhandleApproveClick = (rowData) => {
    navigate(
      `/processsheetview?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.project_id)}&pm_processsheet_id=${encryptData(rowData.pm_procsheet_id)}&Proc_type=${encryptData(rowData.Proc_Type)}&ViewType=onhandleApproveClick`
    );
    // setShowTestDate('');
    // setFormData({
  ...
  const onhandleSecondApproveClick = (rowData) => {
    navigate(`/processsheetview?moduleId=${moduleId}&menuId=${menuId}&id=${encryptData(rowData.project_id)}&pm_processsheet_id=${encryptData(rowData.pm_procsheet_id)}&Proc_type=${encryptData(rowData.Proc_Type)}&ViewType=onhandleSecondApproveClick`);
    // setShowTestDate('Yes');
    // setFormData({
    //   ...formData, pm_project_id: rowData.project_id == null ? "0" : rowData.project_id + '',
    //   pm_processsheet_id: rowData.pm_procsheet_id == null ? "0" : rowData.pm_procsheet_id + '',
                  ...
                  <div className="ProcessSheetlistTable">
                    ...
                    <div className="tableheaderflex">
                      ...
                      <div className="tableheaderAddbutton">
                        ...
                        {permissions?.createPerm ? <Link style={{ float: "right" }} to={`/coating?moduleId=${moduleId}&menuId=${menuId}`} target="_blank">
