...
function Viewtallytagmapping() {
    ...
    const decTallySheetId = decryptData(tallySheetId)
    const handleSubmit = (event) => {
        event.preventDefault();

        const dataToSend = fileData.pipeDetails?.map(pipe => ({
            pipeId: pipe.pipeNo,
            tallySheetId: parseInt(decTallySheetId),
        ...
        axios.post(`${Environment.BaseAPIURL}/api/User/UpdatePipeStatus`, dataToSend, {
            ...
            .then(response => {
                ...
                navigate(`/tallytagmappinglist?moduleId=${moduleId}&menuId=${menuId}`);
            })
            .catch(error => {
                            ...
                            <div className="row">
                                <div className="col-md-12 col-sm-12 col-xs-12">
                                    <ul>
                                        ...
                                        <li><h1>/&nbsp; Tally Tag Mapping Edit</h1></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
