...
function Dailyproductionreport() {
  ...
  const pm_procSheet_id = searchParams.get('procsheetID');
  ...
  const getHeaderData = async () => {
    try {
      const response = await axios.post(`${Environment.BaseAPIURL}/api/User/getEPOXYProcessSheetDetails?processsheetno=${action != 'edit' ? encryptData(formData.psSeqNo) : pm_procSheet_id}&year=${action != 'edit' ? encryptData(formData.psYear) : encryptData('2024')}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetDPRDataByID?p_procsheet_id=${encryptData(response.data.Table[0].processsheetid)}&p_dpr_id=${id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        navigate(`/dailyproductionreportlist?moduleId=${moduleId}&menuId=${menuId}`)
        setVisible(true)
      }
    } catch (error) {
      console.error('Error fetching process sheet details:', error);
                ...
                <div className='row'>
                  <div className='col-md-12 col-sm-12 col-xs-12'>
                    <ul>
                      ...
                      <li><b style={{ color: '#fff' }}>/&nbsp;</b> <Link to={`/dailyproductionreportlist?moduleId=${moduleId}&menuId=${menuId}`}>Daily Production Report List</Link></li>
