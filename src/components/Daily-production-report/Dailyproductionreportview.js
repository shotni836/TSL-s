...
function Dailyproductionreport() {
  ...
  const procsheetID = searchParams.get('procsheetID');
  const pm_Approve_level = searchParams.get('pm_Approve_level');
  ...
  const procsheetID1 = decryptData(procsheetID)
  const id1 = decryptData(id)
  const [formData, setFormData] = useState({
    pm_comp_id: 1,
    pm_location_id: 1,
    pm_processSheet_id: parseInt(procsheetID1),
    pm_remarks: "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetInspectedByAcceptedByDetails?matid=${encryptData('0')}&testId=${id}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const callWitness = async () => {
    const response1 = await axios.post(`${Environment.BaseAPIURL}/api/User/GetEmployeeTypeWithName?p_procsheet_id=${procsheetID}&p_test_run_id=${encryptData(3421)}&p_type_id=${id}`, {}, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (responseBody === '100' || responseBody === '200') {
        ...
        navigate(`/dailyproductionreportlist?moduleId=${moduleId}&menuId=${menuId}&testingtype=${encryptData(`2233`)}`)
