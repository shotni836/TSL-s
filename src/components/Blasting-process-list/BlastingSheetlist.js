...
function BlastingSheetlist() {
    ...
    const menuId1 = queryParams.get('menuId');
    const menuId = decryptData(menuId1);
    ...
    const getProcessUrl = (action, data) => {
        ...
        return `${basePath}&${encryptData(data.pm_test_run_id)}&pm_Approve_level=${action}&pm_project_id=${encryptData(data?.project_id)}&pm_processSheet_id=${encryptData(data?.procsheet_id)}&pm_processtype_id=${encryptData(typeWise())}&pm_approved_by=${encryptData(userId)}&test_date=${encryptData(data?.test_date)}&menuId=${menuId1}`;
    };

    // const handleApprovalDust = (data, action) => {
    //     return `/dustlevelview/${data.pm_blasting_test_run_id}&pm_Approve_level=${action}&pm_project_id=${encryptData(data?.project_id)}&pm_processSheet_id=${encryptData(data?.procsheet_id)}&pm_processtype_id=${typeWise()}&pm_approved_by=${encryptData(userId)}&test_date=${encryptData(data?.test_date)}&menuId=${menuId}`;
    // };

    const addCalibration = (action, data) => {
        const basePath = '/calibration-blasting-report';
        return `${basePath}?year=${data.pm_procsheet_year}&TestRunId=${encryptData(data.pm_test_run_id)}&pm_blasting_test_run_id=${data.pm_blasting_test_run_id}&pm_Approve_level=${action}&pm_project_id=${encryptData(data?.project_id)}&ProcessSheetID=${encryptData(data?.procsheet_id)}&ProcessSheetTypeID=${encryptData(typeWise())}&pm_approved_by=${encryptData(userId)}&test_date=${encryptData(data?.test_date)}&moduleId=${moduleId}&menuId=${menuId1}`;
    };

    const viewCalibration = (action, data) => {
        const basePath = '/calibration-blasting-report-view';
        return `${basePath}/${encryptData(data.pm_test_run_id)}&pm_Approve_level=${action}&pm_project_id=${encryptData(data?.project_id)}&pm_processSheet_id=${encryptData(data?.procsheet_id)}&pm_processtype_id=${encryptData(typeWise())}&pm_approved_by=${encryptData(userId)}&test_date=${encryptData(data?.test_date)}&moduleId=${moduleId}&menuId=${menuId1}`;
    };
    const handleApprovalCalib = (data, action) => {
        return `/calibration-blasting-report-view/${encryptData(data.pm_test_run_id)}&pm_Approve_level=${action}&pm_project_id=${encryptData(data?.project_id)}&pm_processSheet_id=${encryptData(data?.procsheet_id)}&pm_processtype_id=${encryptData(typeWise())}&pm_approved_by=${encryptData(userId)}&test_date=${encryptData(data?.test_date)}&moduleId=${moduleId}&menuId=${menuId1}`;
    };
    ...
    const fetchPermissions = async () => {
        try {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetPermissionDetailsByPageId`, {
                params: { UserId: encryptData(userId), PageId: menuId1 },
                headers: {
    ...
    const fetchProcessSheetDetails = async () => {
        ...
        try {
            ...
            response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetProcessSheetDetailsList`, {
                params: { ProcessType: encryptData(typeWise()), UserId: encryptData(userId) },
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
            ...
            if (['24', '26'].includes(menuId)) {
                response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetCalibRoughnessList`, {
                    params: { ProcessType: menuId === '24' ? encryptData('1398') : menuId === '26' ? encryptData('1400') : encryptData(typeWise()), UserId: encryptData(userId) },
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    }
                });
            ...
            if (menuId === '25') {
                response2 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetDustlevelTestList`, {
                    params: { ProcessType: menuId === '26' ? encryptData('1399') : encryptData(typeWise()), testid: encryptData('237'), UserId: encryptData(userId) },
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    }
                });
                        ...
                        <div className="container">
                            <div className="ProcessDataEntryListBox">
                                {menuLinks.map(({ id, label, name }) => (
                                    <Link key={id} title={`${name}`} to={`/blastingsheetlist?moduleId=${moduleId}&menuId=${encryptData(id)}`} className={currentId === id ? 'current-process-selection' : ''} >{label}</Link>
                                ))}
                                <Link to={`/list/miseleaneous-test?moduleId=${moduleId}&menuId=${menuId1}`} style={{
                                    display: 'block',
                                    background: '#484848',
                                    padding: '4px 16px',
                                    color: '#fff',
                                ...
                                <Link to={`/repair-list?moduleId=${moduleId}&menuId=${menuId1}`} style={{
                                    display: 'block',
                                    background: '#484848',
                                    padding: '4px 16px',
                                    color: '#fff',
                                    ...
                                    <div className="ProcessSheetlistTable">
                                        <div className="tableheaderflex">
                                            ...
                                            {permissions?.createPerm === "1" &&
                                                ...
                                                <Link style={{ float: 'right' }} to={`/blastingprocesssheet?moduleId=${moduleId}&menuId=${menuId1}&id=${encryptData(typeWise())}`} target="_blank">
                                                    <i className="fas fa-plus"></i> Add
                                                </Link>
                                            </div> : menuId !== '25' ? <div className="tableheaderAddbutton">
                                                <Link style={{ float: 'right' }} to={`/calibration-blasting-report?moduleId=${moduleId}&menuId=${menuId1}&id=${encryptData(typeWise())}&action=add`} target="_blank">
                                                    <i className="fas fa-plus"></i> Add
                                                </Link>
                                            </div> : ""}
                                        </div>
                                                                                ...
                                                                                <td>
                                                                                    <div className="action-icons">
                                                                                        ...
                                                                                        {permissions?.editPerm === "1" && (item.IsSubmitted === 1 || item.pm_is_superincharge === 1) && (
                                                                                            <i className="fas fa-edit" onClick={() => navigate(`/calibration-blasting-report?action=edit&year=${encryptData(item.pm_procsheet_year)}&ProcessSheetID=${encryptData(item.procsheet_id)}&ProcessSheetTypeID=${encryptData(typeWise())}&TestRunId=${encryptData(item.pm_test_run_id)}&pm_blasting_test_run_id=${encryptData(item.pm_blasting_test_run_id)}&moduleId=${moduleId}&menuId=${menuId1}`)} style={{ color: "#ff9800", margin: '4px', cursor: "pointer" }}></i>
                                                                                        )}
                                                                                    </div>
                                                                                </td>
                                                                            </tr>
