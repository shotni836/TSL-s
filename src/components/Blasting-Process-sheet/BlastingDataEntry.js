...
const BlastingDataEntry = () => {
  ...
  async function editData(data_procedure1) {
    try {
      ...
      const response2 = await axios.post(`${Environment.BaseAPIURL}/api/User/GetInspectionRandomWitnessById?ProcessSheetID=${procsheetId}&ProcessSheetTypeID=${value_type}&testid=${pm_test_run_id}&current_companyid=${currentCompanyId}`)
      ...
      const response4 = await axios.post(`${Environment.BaseAPIURL}/api/User/GetEmployeeTypeWithName?p_procsheet_id=${procsheetId}&p_test_run_id=${pm_test_run_id}&p_type_id=${value_type}`);
      setWitnessData(response4?.data)

      const response5 = await axios
        .get(
          Environment.BaseAPIURL +
          `/api/User/GetDestinationDetails?pm_procsheet_id=${response.data[0][0].procsheet_id}`
        )
    ...
    try {
      ...
      const response2 = value_type != 528 ? await axios
        .get(
          ...
          .get(
            Environment.BaseAPIURL +
            `/api/User/GetExternalFinalDataPendingPipe?pm_process_type_id=${528}&pm_procsheet_id=${data1}`
          )
  ...
  const getTestBlastingDataEntries = async (value) => {
    try {
      ...
      const response = await axios
        .get(
          Environment.BaseAPIURL +
          `/api/User/GetTestBlastingdataEntryNewtmp?processsheetno=${formData.type}&processtype=${value}&TestDate=${formData.testdate}&shiftid=${currentShift}`
        )
      // rowData = []
      ...
      const response1 = await axios
        .get(
          Environment.BaseAPIURL +
          `/api/User/GetDestinationDetails?pm_procsheet_id=${formData.procsheet_id}`
        )
                        ...
                        <>
                          ...
                          <div className="col-md-12 col-sm-12 col-xs-12">
                            <div className="d-flex align-items-center">
                              ...
                              {typeOfProcess == 524 ? <><a href={`/dustlevelview/0&pm_processSheet_id=${formData.procsheet_id}&test_id=237`} target="_blank" className="btn btn-primary mx-2" style={{ padding: '4px' }}>Dust Level Report</a></> : ''}
                              {typeOfProcess == 524 ? <><a href={`/calibration-blasting-report?year=${formData.year}&ProcessSheetID=${formData.procsheet_id}&ProcessSheetTypeID=1398&menuId=${menuId}`} target="_blank" className="btn btn-primary mx-2" style={{ padding: '4px' }}>Calibration</a></> : ''}
                            </div>
                          </div>
