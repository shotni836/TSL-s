...
function RepairList() {
    ...
    const handleViewClick = (row) => {
        navigate(`/repair-report/moduleId=${moduleId}&menuId=${menuId}&pm_Approve_level=view&pm_processSheet_id=${encryptData(row.procsheet_id)}&pm_test_run_id=${encryptData(row.pm_test_run_id)}`);
    };
                                        ...
                                        <div className='tableheaderflex'>
                                            ...
                                            <div className='tableheaderAddbutton'>
                                                {permissions?.createPerm === "1" && (
                                                    <Link to={`/repair-add?moduleId=${moduleId}&menuId=${menuId}`} target='_blank'>
