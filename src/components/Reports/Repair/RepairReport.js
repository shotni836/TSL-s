...
function RepairReport() {
    ...
    let testRunId2 = null;
    let pm_processSheet_id2 = null;
    let pm_processtype_id2 = null;
    let pm_approved_by2 = null;
    ...
    for (let i = 0; i < pathSegments.length; i++) {
        if (pathSegments[i].startsWith('pm_test_run_id=')) {
            testRunId2 = pathSegments[i].substring('pm_test_run_id='.length);
            testRunId = decryptData(testRunId2)
        }
        if (pathSegments[i].startsWith('pm_processSheet_id=')) {
            pm_processSheet_id2 = pathSegments[i].substring('pm_processSheet_id='.length);
            pm_processSheet_id1 = decryptData(pm_processSheet_id2)
        }
        if (pathSegments[i].startsWith('pm_processtype_id=')) {
            pm_processtype_id2 = pathSegments[i].substring('pm_processtype_id='.length);
            pm_processtype_id1 = decryptData(pm_processtype_id2)
        }
        if (pathSegments[i].startsWith('pm_approved_by=')) {
            pm_approved_by2 = pathSegments[i].substring('pm_approved_by='.length);
            pm_approved_by1 = decryptData(pm_approved_by2)
        }
        ...
        const fetchData = async () => {
            try {
                if (tstmaterialid) {
                    ...
                    const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetRepairReport?ProcessSheetID=${pm_processSheet_id2}&ProcessSheetTypeID=${encryptData(1428)}&TestRunId=${testRunId2}&TestId=${encryptData(0)}`, {
                        headers: {
                    ...
                    const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GETInstrumentDetails?TestRunId=${testRunId2}&ProcessSheetId=${pm_processSheet_id2}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                        }
                    });
                ...
                try {
                    if (tstmaterialid) {
                        const [id1, id2, id3, id4] = tstmaterialid.split('&');
                        const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetInspectedByAcceptedByDetails?matid=${encryptData(609)}&testId=${encryptData(1)}`, {
                            headers: {
                                'Authorization': `Bearer ${token}`,
                            }
                        });
    ...
    async function callWitness() {
        try {
            const [id1, id2] = tstmaterialid.split('&');
            const response1 = await axios.post(`${Environment.BaseAPIURL}/api/User/GetEmployeeTypeWithName?p_procsheet_id=${pm_processSheet_id2}&p_test_run_id=${testRunId2}&p_type_id=${encryptData(609)}`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
