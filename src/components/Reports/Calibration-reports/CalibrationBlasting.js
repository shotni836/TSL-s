...
const CalibrationBlasting = () => {
    ...
    let procSheetId1 = searchParams.get('ProcessSheetID');
    let procSheetId = decryptData(procSheetId1)
    let TestRunId1 = searchParams.get('TestRunId');
    let TestRunId = decryptData(TestRunId1)
    let ProcessSheetTypeID1 = searchParams.get('ProcessSheetTypeID');
    let ProcessSheetTypeID = decryptData(ProcessSheetTypeID1)
    let year1 = searchParams.get('year');
    let year = decryptData(year1)
    ...
    const fetchEditDetails = async () => {
        const response = TestId == '325' ? await axios.post(Environment.BaseAPIURL + `/api/User/GetCalibrationblastingdataById?ProcessSheetID=${procSheetId1}&ProcessSheetTypeID=${ProcessSheetTypeID1}&TestRunId=${TestRunId1}&TestId=${encryptData(TestId)}`, {
            headers: {
                'Authorization': `Bearer ${token}`,
            }
        })
            : await axios.post(Environment.BaseAPIURL + `/api/User/GetCalibrationblastingdataById?ProcessSheetID=${procSheetId1}&ProcessSheetTypeID=${ProcessSheetTypeID1}&TestRunId=${TestRunId1}&TestId=${encryptData(TestId)}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            })
    ...
    const getHeaderData = async (data) => {
        try {
            const response = await axios.post(`${Environment.BaseAPIURL}/api/User/getEPOXYProcessSheetDetails?testingtype=${ProcessSheetTypeID1}&year=${year1}&processsheetno=${procSheetId1}`, {}, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
            ...
            const response1 = await axios.get(`${Environment.BaseAPIURL}/api/User/GetCalibrationblastingdata?processsheetno=${procSheetId1}&testid=${ProcessSheetTypeID1}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
            ...
            const response2 = await axios.get(Environment.BaseAPIURL + `/api/User/GetWiTestList?test_id=${ProcessSheetTypeID1}&mater_id=${encryptData(0)}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
    ...
    const handleSubmit = async (e, value) => {
        ...
        try {
            ...
            ProcessSheetTypeID == '608' ? navigate(`/rawmateriallist?menuId=${encryptData(24)}&testingtype=${encryptData(608)}`) : navigate(`/blastingsheetlist?moduleId=${moduleId}&menuId=${menuId}`);
            if (response.data === 1000) {
                toast.success("Data saved successfully!");
                console.log("Form data sent successfully!");
                navigate(`/inspectiontesting?menuId=${encryptData(24)}`);
            } else {
