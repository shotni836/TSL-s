...
function Nc() {
    ...
    const procsheetId1 = searchParams.get("processSheetId");
    ...
    const getYear = async () => {
        ...
        if (action == 'edit') {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetNCDatabyid?ProcessSheetID=${procsheetId1}&nc_run_id=${id1}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
