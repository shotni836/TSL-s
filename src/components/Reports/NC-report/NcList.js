...
function NcList() {
    ...
    const getList = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetNCDataList?userid=${userId}`);
    ...
    const handleViewClick = (row) => {
        navigate(`/nc-report?action=edit&id=${row.pm_nc_run_id}&processSheetId=${row.procsheet_id}&menuId=${38}`, '_blank');
    };
    const handleEdit = (row) => {
        navigate(`/nc-add?action=edit&id=${row.pm_nc_run_id}&processSheetId=${row.procsheet_id}&menuId=${38}`, '_blank');
    };
