    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (tstmaterialid) {
                    const [id1, id2, id3, id4] = tstmaterialid.split('&');
                    const response = id4 != 987 || id4 != 1433 ? await axios.post(`${Environment.BaseAPIURL}/api/User/GetFlexibilityTestReport_ForMtc?pm_procsheet_id=${id1}&pipecode=${id2}&rtype=${609}&testid=${292}`)
                ...
                try {
                    if (tstmaterialid) {
                        const [id1, id2, id3, id4] = tstmaterialid.split('&');
                        const response = await axios.get(`${Environment.BaseAPIURL}/api/User/GetInspectedByAcceptedByDetails?matid=${609}&testId=${292}`);
                    ...
                    <div>
                        ...
                        <div ref={contentRef}>
                            {chunkedData.map((chunk, chunkIndex) => (
                                <div key={chunkIndex} className={`InspReportSection ${classtodefine}`} ref={contentRef}>
...
export default ImpactTestReports;