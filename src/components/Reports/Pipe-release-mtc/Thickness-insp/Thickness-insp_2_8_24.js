    ...
    const fetchData = async () => {
      try {
        if (tstmaterialid) {
          ...
          console.log(data._CdTesHeaderDetails[0].poNo, "test")
          const { key, value } = parseKeyValuePair(data._CdTesHeaderDetails[0].poNo);
          setKey(key);
          setKeyValue(value)
          console.log(key, value, 140)
        }
      } catch (error) {
                            ...
                            <div className='row'>
                              <div className='col-md-12 col-sm-12 col-xs-12 p-0'>
                                <table>
                                  <tbody>
                                    <tr>
                                      <td>ABOVE RESULTS ARE CONFORMING TO SPECIFICATION : <span style={{ fontFamily: 'Myriad Pro Light' }}>{headerDetails.current.specification || "-"} & {headerDetails.current.acceptanceCriteria || "-"} AND FOUND SATISFACTORY.</span></td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
