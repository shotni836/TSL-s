...
function BarePipeInsp() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const nameEmpCount = data._CreatedEmployeeName.reduce((acc, item) => {
                    const key = `${item.EmployeeName}-${item.emp_id}`;
                    acc[key] = (acc[key] || 0) + 1;
                    return acc;
                }, {});
                ...
                for (const key in nameEmpCount) {
                    if (nameEmpCount[key] > maxCount) {
                        maxCount = nameEmpCount[key];
                        mostFrequentCombination = key;
                    }
                }

                // Filter the data for the most frequent combination of EmployeeName and emp_id
                const mostFrequentData = data._CreatedEmployeeName.find(item => `${item.EmployeeName}-${item.emp_id}` === mostFrequentCombination);

                // Check if the data fields are empty or missing and apply fallback
                const outputData = {
                    pm_test_run_id: mostFrequentData?.pm_test_run_id ?? null,
                    pm_process_type_id: mostFrequentData?.pm_process_type_id ?? null,
                    pm_pipe_id: mostFrequentData?.pm_pipe_id ?? null,
                    pm_pipe_code: mostFrequentData?.pm_pipe_code ?? "Unknown",
                    pm_Approve_level: mostFrequentData?.pm_Approve_level ?? 0,
                    emp_id: mostFrequentData?.emp_id ?? "Unknown",
                    EmployeeName: mostFrequentData?.EmployeeName ?? "Unknown",
                    emp_sign_filename: mostFrequentData?.emp_sign_filename ?? "No Signature",
                    pm_test_accepted: mostFrequentData?.pm_test_accepted ?? 0,
                    Designation: mostFrequentData?.Designation ?? "Not Available",
                    Department: mostFrequentData?.Department ?? "Not Available",
                    CompanyName: mostFrequentData?.CompanyName ?? "Not Available",
                    ApproveDate: mostFrequentData?.ApproveDate ?? "Not Available",
                    pm_test_fb_date: mostFrequentData?.pm_test_fb_date ?? "Not Available",
                    Status: mostFrequentData?.Status ?? "Not Available",
                    Remarks: mostFrequentData?.Remarks ?? "No Remarks",
                    RandomWitnessImage: mostFrequentData?.RandomWitnessImage ?? "No Image",
                    WitnessImage: mostFrequentData?.WitnessImage ?? "No Image"
                };
                ...
                console.log(data._RandomWitness)
    ...
    const handleStatusChange = (value) => {
        setIsClicked(true)
        console.log('okokokok', witnessValue)
        if (value === "A") {
