...
function FInalInsp() {
  ...
  const combineDataWithWitnesses = (data, witnessesByPipeCode) => {
    console.log(witnessesByPipeCode, "data2")
    return data.map(pipe => {
      const pipeCode = pipe["Pipe No."].value;
      console.log(witnessesByPipeCode[pipeCode], "data4")
      return {
        ...pipe,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${Environment.BaseAPIURL}/api/User/Get_ExternalFinalReport?matid=${528}&testId=${id2}`);
                                        ...
                                        <table className='BarePipeInspCustomTable'>
                                          ...
                                          <tbody>
                                            {chunk?.map((row, rowIndex) => {
                                              ...
                                              console.log(testOnPipe, "testOnPipe");
                            ...
                            <div className='CustomBarePipeWitnessBox'>
                              ...
                              <div className='CustomBareWitnessBox'>
                                ...
                                <div>
                                  ...
                                  {chunk?.map((row, rowIndex) => {
                                    const hasData = Object.values(row).some(value => value !== undefined && value !== null && value !== '-');
                                    console.log(witnessesByPipeCode[row["Pipe No."]?.value], row["Pipe No."]?.value, "test1");
